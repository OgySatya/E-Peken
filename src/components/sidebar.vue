...
function sortName() {
  ...
  if (ascName.value === true) {
    store.allProduct = store.filteredName
      .slice()
...
<template>
  <div class="flex h-max mx-2">
    <div class="left-0 flex-row-reverse hidden md:block">
      <transition>
        <div
          :class="[open ? 'flex justify-between' : 'grid gap-5']"
      ...
      <div
        class="transition-all duration-700 bg-neutral-200 overflow-hidden flex justify-center rounded-lg"
